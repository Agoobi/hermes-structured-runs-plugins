...
from _plugin import _config, _finalize, _media, _session_db
...
class FinalOutputCheckPromptTests(unittest.TestCase):
    def test_includes_arbitrary_client_schema_and_media_contract(self) -> None:
        ...
        prompt = _finalize.final_output_check_prompt(schema)
    ...
    def test_extracts_output_from_all_supported_run_shapes(self) -> None:
        self.assertEqual(_finalize.run_output_text({"output": "first"}), "first")
        self.assertEqual(_finalize.run_output_text({"final_output": "second"}), "second")
        self.assertEqual(_finalize.run_output_text({"result": "third"}), "third")

    def test_falls_back_to_json_for_missing_output(self) -> None:
        output = _finalize.run_output_text({"status": "completed", "run_id": "run_123"})
        self.assertIn('"run_id": "run_123"', output)

    def test_wrapper_resolves_relative_artifact_without_agent_cwd(self) -> None:
        original_roots = _config.MEDIA_ROOTS
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            artifact = root / "videos" / "demo" / "output" / "video.mp4"
            artifact.parent.mkdir(parents=True)
            artifact.write_bytes(b"not-a-real-video")
            _config.MEDIA_ROOTS = [root]
            try:
                text = "File: videos/demo/output/video.mp4"
                self.assertEqual(_media.verified_artifacts_from_text(text), [str(artifact)])
                self.assertEqual(
                    _media.resolve_media_path("MEDIA:videos/demo/output/video.mp4"), artifact
                )
                parsed = _media.canonicalize_artifact_paths(
                    {"media_path": "MEDIA:videos/demo/output/video.mp4"}
                )
                self.assertEqual(parsed["media_path"], str(artifact))
            finally:
                _config.MEDIA_ROOTS = original_roots

    def test_session_state_tracks_pending_delegation_and_latest_reply(self) -> None:
        original_db = _config.STATE_DB
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                state = _session_db.session_work_state("run_test")
                self.assertEqual(state["pending_delegations"], 1)
                self.assertEqual(state["pending_delivery"], 0)
                self.assertEqual(_session_db.latest_session_output("run_test")["output"], "new final")
            finally:
                _config.STATE_DB = original_db
